...
class GaussianBeam(object):
    ...
    def E_field_amplitude(
        ...
        if not isinstance(self.w0, Sequence): # Circular beam shape
    ...
    def _calculate_rotation_matrix(self,):
        ...
        assert np.linalg.norm(self._beam_direction) == 1, "beam_direction must be a unit vector"

        # Choosing local x-axis based on the beam direction
        if self._beam_direction[0] != 0:
            local_x_axis = np.array([0, 1, 0])
        else:
            local_x_axis = np.array([1, 0, 0])

        # Computing local y-axis
        local_y_axis = np.cross(self._beam_direction, local_x_axis)
        local_y_axis /= np.linalg.norm(local_y_axis)  # Normalize

        # Adjust local x-axis to ensure orthogonality
        local_x_axis = np.cross(local_y_axis, self._beam_direction)

        # Constructing rotation matrix
        R = np.column_stack((local_x_axis, local_y_axis, self._beam_direction))
